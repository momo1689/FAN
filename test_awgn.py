...
parser.add_argument('--ckpt', type=str, default='./checkpoint/model_awgn',
                    help="Checkpoint path")
parser.add_argument('--img_dir', type=str, default='./data/LIVE1/',
                    help="input directory of images with synthetic noise")
parser.add_argument('--sigma', type=int, default=50,
                    help="Noise level (15 - 50)")
...
img_paths = [str(i) for i in Path(args.img_dir).glob('*') if i.suffix.lower() in img_suffixes]
for img_path in img_paths:
    img_gt = cv2.cvtColor(cv2.imread(img_path), cv2.COLOR_BGR2RGB) / 255
    ...
    img_noisy = random_noise(img_gt, 'gaussian', mean=0, var=(args.sigma/255)**2)
